...
def main() -> None:
    ...
    if command == "euclidean-grid-stride":
        ...
        plt.imshow(image)
        plt.show()
    elif command == "euclidean-warp-shfl":
        image = voronoi_euclidean_warp_shfl(
            points=generate_uniform_points(point_count=point_count),
            resolution=resolution
        )
        plt.imshow(image)
...
def voronoi_euclidean_grid_stride(
    ...
    _voroni_euclidean_grid_stride_kernel[blocks_per_grid, threads_per_block]( # type: ignore
        points,
        out_image
    )
...
def voronoi_euclidean_warp_shfl(
    points: cuda.devicearray.DeviceNDArray,
    resolution: int
) -> np.ndarray:
    out_image = make_empty_voronoi_output(resolution=resolution)
    ...
    _voroni_euclidean_warp_shfl[blocks_per_grid, threads_per_block]( # type: ignore
        points,
        out_image
    )
