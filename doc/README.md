...
- Performance-Vergleichsplot (`create_kernel_performance_plot`): Dieses Diagramm zeigt die Kernel-Laufzeit in Abhängigkeit von der Punkteanzahl bei einer **festen** Bildauflösung. Sowohl die X-Achse (Input-Größe $N$) als auch die Y-Achse (Laufzeit in ms) nutzen eine logarithmische Skalierung. Da die Funktion eine Liste von Kernel entgegennehmen kann, ist es möglich, mithilfe dieses Plots mehrere Kernel-Implementierungen direkt miteinander zu vergleichen.

- Performance-Matrix / Heatmap (`create_kernel_performance_matrix`): Da die Laufzeit von zwei Eingabeparameter (Bildauflösung und Punkteanzahl) maßgeblich abhängt, zeigt diese Matrix die gegenseitige Beeinflussung der beiden Parameter. Die Zeilen repräsentieren die Auflösung, die Spalten die Punkteanzahl. In jeder Zelle wird die konkrete Laufzeit angezeigt.
...
| ![](../data/performance_plot_NVIDIA-GeForce-RTX-5070_naive_euclidean_hypot_warp_shfl_euclidean_hypot_resolution=128_points=64,128,256,512.png) | ![](../data/performance_plot_NVIDIA-GeForce-GTX-1660-Ti_naive_euclidean_hypot_warp_shfl_euclidean_hypot_resolution=128_points=64,128,256,512.png) |
...
1. Die Threads eines Blocks werden nach der X-Dimension zu Warps zusammengefasst, wodurch ein Warp einer horizontalen Gridzeile entspricht. Shuffle kann dann grundsätzlich nur auf die Pixel $P+k$ und $P-k$ **nach rechts bzw. links** zugreifen. Die Nachbarn oben, unten und auf den vier Diagonalen liegen per Definition in anderen Zeilen und damit außerhalb des Warps. Dafür bleibt zwingend ein Fallback auf Global Memory nötig, wodurch Shuffle im besten Fall also nur 2 der 8 Nachbarschaftsprüfungen betreffen würde (`dy == 0 and dx == +/-1`).
   - Anmerkung: Bei einer Blockkonfiguration, bei der `blockDim.x` **kein** Vielfaches von 32 ist - wie in diesem Fall für `BLOCK_DIM = 16` - verschiebt sich dieses Bild leicht: Ein Warp fasst dann zwei volle Zeilen zusammen ($2 \cdot 16 = 32$), sodass für einen Teil der Threads sogar der direkt vertikale Nachbar im selben Warp läge. In der Vertikalen gewinnt man zwar einen Schritt, in der Horizontalen verliert man jedoch die Hälfte. Zudem ändert das nichts am grundsätzlichen Problem, da weiterhin nur benachbarte Zeilenpaare betroffen sind (nicht beliebige y-Distanzen).
